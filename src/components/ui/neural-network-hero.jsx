    ...
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <div
        ...
        <div
          ...
          <span
            className="text-[10px] font-light uppercase tracking-[0.08em] text-white/70" style={{fontFamily: 'var(--font-jetbrains-mono)'}}>{badgeLabel}</span>
          <span className="h-1 w-1 rounded-full bg-white/40" />
          <span className="text-xs font-light tracking-tight text-white/80" style={{fontFamily: 'var(--font-jetbrains-mono)'}}>{badgeText}</span>
        </div>

        <h1
          ref={headerRef}
          className="max-w-4xl text-left text-4xl font-thin leading-[0.95] tracking-[-0.02em] text-white sm:text-5xl md:text-6xl lg:text-7xl" style={{fontFamily: 'var(--font-jetbrains-mono)'}}>
          {title}
        </h1>

        <p
          ref={paraRef}
          className="max-w-2xl text-left text-lg font-light leading-relaxed tracking-wide text-white/80 sm:text-xl md:text-2xl mt-8" style={{fontFamily: 'var(--font-jetbrains-mono)'}}>
          {description}
        </p>
        ...
        <ul
          ref={microRef}
          className="mt-12 flex flex-wrap gap-8 text-sm font-light tracking-wider text-white/70">
          {microDetails.map((detail, index) => {
            const refMap = [microItem1Ref, microItem2Ref, microItem3Ref];
            return (
              <li key={index} ref={refMap[index]} className="flex items-center gap-3">
                <span className="h-1.5 w-1.5 rounded-full bg-blue-400/60" /> {detail}
              </li>
            );
          })}
        </ul>
